...
- [**Series**](#Series)
  - [Получение данных Series](#Получение-данных-Series)
  - [Фильтры](#Фильтры)
  - [Глубокое копирование](#Глубокое-копирование)
  - [Агрегация](#Агрегация)
  - [Apply](#Apply)
  - [Методы Series](#Методы-Series)
- [**DataFrame**](#DataFrame)
  - [Получение данных](#Получение-данных)
  - [Получение данных из файла](#Получение-данных-из-файла)
  - [Добавление и удаление колонок](#Добавление-и-удаление-колонок)
  - [Переименование колонок](#Переименование-колонок)
  - [Работа с NaN](#Работа-с-NaN)
  - [Excel файлы](#Excel-файлы)
  - [Работа со строками](#Работа-со-строками)
  - [Группировка](#Группировка)
  - [Сортировка](#Сортировка)
  - [Объединение DataFrame](#Объединение-DataFrame)
...
s['Колонка'].str.capitalize() # Начало с заглавной буквы
s['Колонка'].str.title() # Каждое слово с большой буквы
s['Колонка'].str.swapcase() # Меняет регистр
```

### Группировка

```python
s.groupby('Колонка, по которой группировать').count('Колонка, к которой применима функция')
```

В примере идет подсчет значений.
...
|Атрибут|Описание
|---|---
|ascending=True|Порядок сортировки
|na_position=last, first|Положение в выдаче NaN .
|ignore_index=True|Игнорировать индексы при сортировке.

### Объединение DataFrame

Объединение DataFrame выглядит так.

```python
s1=pd.read_excel('Файл01')
s2=pd.read_excel('Файл02')
sfull=pd.concat([s1,s2])
```
...
|Атрибут|Описание
|---|---
|ignore_index=True|Игнорирует индексы
|keys=[]|В скобках можно передать спистк ключей для каждого DataFrame.

### Сводные таблицы

Сводные таблицы применяются часто для получения значений на основе существующих, например среднее значение. Это своего рода удобочитаемая группировка.

```python
s.pivot_table(index='Колонка',values='Колонка')
```
...
|Атрибут|Описание
|---|---
|index='Колонка'|Группируемая колонка
|columns='Колонка'|По какой колонке группировать
|aggfunc='func'|Агрегирующая функция

## Визуализация данных

Для визуализации будет использоваться модуль Matplotlib.
...
#### Пример создания графика. По аналогии можно и с другими типами

```python
import matplotlib.pyplot as plt
s=plt.plot([Названия],[Значения])
plt.show()
```

Подробно про Matplotlib будет написано в другой статье.